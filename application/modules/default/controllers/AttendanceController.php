...
class Default_AttendanceController extends Zend_Controller_Action
{
   ...
   public function init()
    	{			
			...
			$this->view->menuActive = 'Employee Attandance';
        ...
        public function indexAction()
        {
            ...
            $sql_employee = $db->select()->from('tbl_employee');
            $employee_Id = $db->fetchAll($sql_employee);
            $this->view->employee = $employee_Id;  
...
// print_r( $res);
        ...
        public function selectempAction()
        {
            ...
            if($Emp_id != 'All'){
                ...
                $sql_emp = $db->select()->from('tbl_employee', array('emp_id'))->where('emp_id =?',$Emp_id);
                $empemail = $db->fetchRow($sql_emp);
                $this->view->empno = $empemail;

            }else{
                
                $sql_emp = $db->select()->from('tbl_employee', array('emp_id'));
                $empemail = $db->fetchAll($sql_emp);
                $this->view->empno = $empemail;
               
            }
            // $this->view->empno = $empemail;
            $table = $empemail;
            // echo"<pre>";print_r($empemail);

            $attendancetable = '<table class="table table-flush" id="datatable-buttons" style="margin-top: -66px;">
                    <tr>
                      <th>Emp-Id</th>
                      <th>Present</th>
                      <th>Absent</th>
                    ...
                    if(count($table)> 1){
                     foreach ($table as $i=>$res) { 
                        $emplyeeid = $res['emp_id'];
                        $id = $i+1;
            $attendancetable.=  '<tr>
           
                      <td><input class="form-control" name="empid" id="empid" type="text" value='.$emplyeeid.' style="border: hidden;"></td>
                      <td><input class="form-check-input" type="radio" name="attandance'.$id.'" id="present'.$id.'" value ="P" checked></td>
                      <td><input class="form-check-input" type="radio" name="attandance'.$id.'" id="absent'.$id.'" value ="A" ></td>
                      
                    </tr>';
                     } 
                    }else{
                        $emplyeeid = $table['emp_id'];
                        $attendancetable.=  '<tr>
                                  <td><input class="form-control" name="empid" id="empid" type="text" value='.$emplyeeid.' style="border: hidden;"></td>
                                  <td><input class="form-check-input" type="radio" name="attandance" id="flexRadioDefault1" value ="P" checked></td>
                                  <td><input class="form-check-input a" type="radio" name="attandance" id="flexRadioDefault2" value ="A"></td>
                                  
                                </tr>';
                    }
            $attendancetable .=  '</table>';
            
            die(json_encode(array('data' => $attendancetable)));
