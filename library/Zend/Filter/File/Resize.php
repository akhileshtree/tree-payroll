 ...
 * @package    Zend_Filter
 */
class Zend_Filter_File_Resize implements Zend_Filter_Interface
{
     ...
     * @return Zend_Filter_File_Resize An instance of this filter
     */
    public function __construct($options = array())
    {
        ...
        if (isset($options['adapter'])) {
            if ($options['adapter'] instanceof Zend_Filter_File_Resize_Adapter_Abstract) {
                $this->_adapter = $options['adapter'];
            } else {
                $name = $options['adapter'];
                if (substr($name, 0, 33) != 'Zend_Filter_File_Resize_Adapter_') {
                    $name = 'Zend_Filter_File_Resize_Adapter_' . ucfirst(strtolower($name));
                }
                $this->_adapter = $name;
            }
        }
    ...
    {
        if ($this->_adapter instanceof Zend_Filter_File_Resize_Adapter_Abstract) {
            return;
        } else {
